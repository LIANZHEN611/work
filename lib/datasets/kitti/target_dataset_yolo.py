import os
import sys

import torch
from torch.utils.data import DataLoader

import cv2
import lib.datasets.kitti.kitti_eval_python.kitti_common as kitti
import numpy as np
import torch.utils.data as data
from lib.datasets.kitti.kitti_eval_python.eval import get_official_eval_result
from lib.datasets.kitti.kitti_utils import (Calibration, Extrinsics,
                                            affine_transform,
                                            get_affine_transform,
                                            get_objects_from_label)
from lib.datasets.kitti.pd import PhotometricDistort
from lib.datasets.utils import angle2class, class2angles
from PIL import Image, ImageFile
from scipy.spatial.transform import Rotation as R
from utils import box_ops

ImageFile.LOAD_TRUNCATED_IMAGES = True


class TargetDataset(data.Dataset):
    def __init__(self, split, cfg):

        # basic configuration
        self.root_dir = cfg.get('target_root_dir')
        self.dataset = cfg.get('target_dataset_name', 'tum')
        self.split = split
        self.max_objs = 50
        self.class_name = ["Car", "Pedestrian", "Cyclist",  'Van', 'Bus', "BigCar"]
        self.cls2id = {'Car': 0, 'CAR': 0,
                      'Pedestrian': 1, 'PEDESTRIAN': 1,
                      'Cyclist': 2, 'CYCLIST': 2,
                      'Motorcycle': 2, 'MOTORCYCLE': 2,
                      'Van': 3, 'VAN': 3,
                      'Bus': 4, 'BUS': 4,
                      'Truck': 5, 'TRUCK': 5,
                      'Trailer': 5, 'TRAILER': 5,
                      'BigCar': 5, 'BIGCAR': 5}
        self.resolution = np.array([960, 600])  # W * H
        self.use_3d_center = cfg.get('use_3d_center', True)
        self.writelist = cfg.get('writelist', ['Car'])
        self.skip_class_names = set(cfg.get('skip_classes', []) or [])
        # anno: use src annotations as GT, proj: use projected 2d bboxes as GT
        self.bbox2d_type = cfg.get('bbox2d_type', 'anno')
        assert self.bbox2d_type in ['anno', 'proj']
        self.meanshape = cfg.get('meanshape', False)
        self.class_merging = cfg.get('class_merging', False)
        self.use_dontcare = cfg.get('use_dontcare', False)

        if self.class_merging:
            self.writelist.extend(
                ['Van', 'Truck', 'Bus', 'Motorcycle', 'Trailer', 'Pedestrian', 'Cyclist'])
        if self.use_dontcare:
            self.writelist.extend(['DontCare'])

        # data split loading
        if "s110_o" in self.root_dir:
            view_ext = cfg.get('s110_o_extrinsic', None)
        elif "s110_w" in self.root_dir:
            view_ext = cfg.get('s110_w_extrinsic', None)
        elif "s110_n" in self.root_dir:
            view_ext = cfg.get('s110_n_extrinsic', None)
        elif "s110_s" in self.root_dir:
            view_ext = cfg.get("s110_s_extrinsic", None)
        else:
            raise ValueError('Unknown view')

        self.yolo_iou_match_threshold = float(cfg.get('yolo_iou_match_threshold', 0.3))
        self.yolo_fallback_conf = float(cfg.get('yolo_fallback_conf', 0.0))
        raw_boost = cfg.get('yolo_conf_boost_by_cls2id', {}) or {}
        self.yolo_conf_boost_by_cls_id = {int(k): float(v) for k, v in raw_boost.items()}
        
        rotation_matrix  = np.array(view_ext).reshape(4,4)[:3,:3]
        ground_rotation = R.from_matrix(
            rotation_matrix)
        # ground_rotation = R.from_matrix(
        #     (camera_transformation() @ feature_extrinsic_matrix)[:3, :3])
        
        yaw_ground, pitch_ground, roll_ground = ground_rotation.as_euler(
            'zyx', degrees=False)
        
        self.view_ext = np.array([yaw_ground, pitch_ground, roll_ground],dtype=np.float32)

        assert self.split in ['train', 'val', 'trainval', 'test']
        # self.split_file = os.path.join(
            # self.root_dir, 'ImageSets_yolo9', self.split + '.txt')
        self.split_file = os.path.join(
            self.root_dir, 'ImageSets', self.split + '.txt')
        # self.idx_list = [x.strip() for x in open(self.split_file).readlines()]
        self.idx_list = [x.strip() for x in open(self.split_file).readlines() if x.strip()]

        # path configuration
        self.data_dir = os.path.join(
            self.root_dir, 'testing' if split == 'test' else 'training')
        self.image_dir = os.path.join(self.data_dir, 'image_2')
        self.calib_dir = os.path.join(self.data_dir, 'calib')
        # self.label_dir = os.path.join(self.data_dir, 'label_2_yolo_v9')
        self.label_dir = os.path.join(self.data_dir, 'label_2')
        self.gt_label_dir = os.path.join(self.data_dir, 'label_2')
        self.extrinsics_dir = os.path.join(self.data_dir, 'extrinsics')
        self.yolo_label_dir = os.path.join(self.data_dir, 'image_2_yolo', 'labels')
        self.has_yolo_conf = os.path.isdir(self.yolo_label_dir)
        # data augmentation configuration
        self.data_augmentation = True if split in [
            'train', 'trainval'] else False

        self.aug_pd = cfg.get('aug_pd', False)
        self.aug_crop = cfg.get('aug_crop', False)
        self.aug_calib = cfg.get('aug_calib', False)

        self.random_flip = cfg.get('random_flip', 0.5)
        self.random_crop = cfg.get('random_crop', 0.5)
        self.scale = cfg.get('scale', 0.4)
        self.shift = cfg.get('shift', 0.1)

        self.depth_scale = cfg.get('depth_scale', 'normal')

        # statistics
        if self.dataset == 'carla':
            #############################   Carla   ###############################
            # Overall Mean: [0.45303926 0.43145254 0.39440957 1.        ]
            # Overall Standard Deviation: [0.21221164 0.20120451 0.19594808 0.        ]

            self.mean = np.array(
                [0.45303926, 0.43145254, 0.39440957], dtype=np.float32)
            self.std = np.array(
                [0.21221164, 0.20120451, 0.19594808], dtype=np.float32)
            self.cls_mean_size = np.array([[1.5889325974143211, 1.9149214083951036, 4.276534553682457],
                                           [1.740576131103809, 0.38891169859355834, 0.38891169859355834],
                                           [1.6485547876898914, 0.790322758934293, 1.7703334617919204],
                                           [2.4277323432934774, 2.4430386748732063, 6.068817606371639],
                                           [3.260256481155415, 2.9435933401272982, 12.648112271151325],
                                           [3.5331199789537897, 3.0104627639025203, 6.707483320057572]]
                                        #    [3.2232471869812227, 2.855413839502194, 6.863092670309799]]
                                          )
        elif self.dataset == 'tum':
            #############################   Tum Traffic   ###############################
            # Overall Mean: [0.38749358 0.41994163 0.38898486]
            # Overall Standard Deviation: [0.12390958 0.12040959 0.11534844]

            self.mean = np.array(
                [0.38749358, 0.41994163, 0.38898486], dtype=np.float32)
            self.std = np.array(
                [0.12390958, 0.12040959, 0.11534844], dtype=np.float32)
            # self.cls_mean_size = np.array([[1.600738818746034, 1.8974925101649465, 4.251457307939188],
            #                                [1.7714340239912671, 0.7477153762268287, 0.826559432933489],
            #                                [1.6566415500538019, 0.8801722282023738, 1.7309257265877247],
            #                                [3.2177711842218817, 2.843558774762212, 6.909936294615725]])
            self.cls_mean_size = np.array([[1.5889325974143211, 1.9149214083951036, 4.276534553682457],
                                           [1.740576131103809, 0.38891169859355834, 0.38891169859355834],
                                           [1.6485547876898914, 0.790322758934293, 1.7703334617919204],
                                           [2.4277323432934774, 2.4430386748732063, 6.068817606371639],
                                           [3.260256481155415, 2.9435933401272982, 12.648112271151325],
                                           [3.5331199789537897, 3.0104627639025203, 6.707483320057572]]
                                        #    [3.2232471869812227, 2.855413839502194, 6.863092670309799]]
                                          )

        elif self.dataset == 'kitti':
            #############################   Kitti   ###############################
            self.mean = np.array([0.485, 0.456, 0.406], dtype=np.float32)
            self.std = np.array([0.229, 0.224, 0.225], dtype=np.float32)
            self.cls_mean_size = np.array([[1.76255119, 0.66068622, 0.84422524],
                                           [1.52563191462, 1.62856739989, 3.88311640418],
                                           [1.73698127, 0.59706367, 1.76282397]])
        if not self.meanshape:
            self.cls_mean_size = np.zeros_like(
                self.cls_mean_size, dtype=np.float32)

        # others
        self.downsample = 32
        self.pd = PhotometricDistort()
        self.clip_2d = cfg.get('clip_2d', False)

    def get_image(self, idx):
        img_file = os.path.join(self.image_dir, '%06d.jpg' % idx)
        # print('check ',img_file)
        assert os.path.exists(img_file), \
        f"Image file not found: {img_file}\n" \
        f"  image_dir={self.image_dir}\n" \
        f"  idx={idx}\n" \
        f"  Hint: check that training/image_2/ contains '{idx:06d}.jpg' and that " \
        f"ImageSets/train.txt IDs match actual files."
        return Image.open(img_file)  # (H, W, 3) RGB mode

    def get_label(self, idx):
        label_file = os.path.join(self.label_dir, '%06d.txt' % idx)
        assert os.path.exists(label_file)
        return get_objects_from_label(label_file)

    def get_calib(self, idx):
        calib_file = os.path.join(self.calib_dir, '%06d.txt' % idx)
        # print('calib',calib_file)
        try: 
            assert os.path.exists(calib_file)
        except:
            pass
        return Calibration(calib_file)

    def get_extrinsics(self, idx):
        extrinsics_file = os.path.join(self.extrinsics_dir, '%06d.json' % idx)
        assert os.path.exists(extrinsics_file)
        return Extrinsics(extrinsics_file, self.dataset)

    def _load_yolo_dets(self, idx, img_w, img_h):
        if not self.has_yolo_conf:
            return np.zeros((0, 5), dtype=np.float32)
        path = os.path.join(self.yolo_label_dir, '%06d.txt' % idx)
        if not os.path.isfile(path):
            return np.zeros((0, 5), dtype=np.float32)
        rows = []
        with open(path, 'r') as f:
            for line in f:
                parts = line.strip().split()
                if len(parts) < 6:
                    continue
                try:
                    cx, cy, w, h, conf = (float(parts[1]), float(parts[2]),
                                          float(parts[3]), float(parts[4]),
                                          float(parts[5]))
                except ValueError:
                    continue
                x1 = (cx - w / 2.0) * img_w
                y1 = (cy - h / 2.0) * img_h
                x2 = (cx + w / 2.0) * img_w
                y2 = (cy + h / 2.0) * img_h
                rows.append([x1, y1, x2, y2, conf])
        if not rows:
            return np.zeros((0, 5), dtype=np.float32)
        return np.asarray(rows, dtype=np.float32)

    @staticmethod
    def _iou_xyxy(box, boxes):
        if boxes.shape[0] == 0:
            return np.zeros((0,), dtype=np.float32)
        ix1 = np.maximum(box[0], boxes[:, 0])
        iy1 = np.maximum(box[1], boxes[:, 1])
        ix2 = np.minimum(box[2], boxes[:, 2])
        iy2 = np.minimum(box[3], boxes[:, 3])
        iw = np.clip(ix2 - ix1, 0.0, None)
        ih = np.clip(iy2 - iy1, 0.0, None)
        inter = iw * ih
        area_a = max(0.0, (box[2] - box[0])) * max(0.0, (box[3] - box[1]))
        area_b = np.clip(boxes[:, 2] - boxes[:, 0], 0.0, None) * np.clip(boxes[:, 3] - boxes[:, 1], 0.0, None)
        union = area_a + area_b - inter
        return np.where(union > 1e-6, inter / union, 0.0).astype(np.float32)

    def _match_yolo_conf(self, objects, yolo_dets):
        n = len(objects)
        conf = np.full((n,), self.yolo_fallback_conf, dtype=np.float32)
        if yolo_dets.shape[0] == 0:
            return conf
        for i, obj in enumerate(objects):
            ious = self._iou_xyxy(obj.box2d, yolo_dets[:, :4])
            if ious.size == 0:
                continue
            j = int(np.argmax(ious))
            if ious[j] >= self.yolo_iou_match_threshold:
                base = float(yolo_dets[j, 4])
                cls_id = self.cls2id.get(obj.cls_type, -1)
                boost = self.yolo_conf_boost_by_cls_id.get(cls_id, 1.0)
                conf[i] = min(1.0, base * boost)
        return conf

    def eval(self, results_dir, logger):
        logger.info("==> Loading detections and GTs...")
        img_ids = [int(id) for id in self.idx_list]
        dt_annos = kitti.get_label_annos(results_dir, img_ids)
        gt_annos = kitti.get_label_annos(self.gt_label_dir, img_ids)

        #test_id = {'Car': 0, 'Pedestrian': 1, 'Cyclist': 2, 'BigCar': 3}
        test_id = {
            'Car': 0,
            'Pedestrian': 1,
            'Cyclist': 2,
            'Van': 3,
            'Bus': 4,
            'BigCar': 5,
        }

        logger.info('==> Evaluating (official) ...')
        car_moderate = 0
        for category in self.writelist:
            if category not in test_id:
                logger.warning(f"Skipping unsupported category '{category}' in evaluation.")
                continue
            results_str, results_dict, mAP3d_R40 = get_official_eval_result(
                gt_annos, dt_annos, test_id[category])
            if category == 'Car':
                car_moderate = mAP3d_R40
            logger.info(results_str)
        return car_moderate

    def __len__(self):
        return self.idx_list.__len__()

    def __getitem__(self, item):
        #  ============================   get inputs   ===========================
        index = int(self.idx_list[item])  # index mapping, get real data id
        # image loading
        img = self.get_image(index)

        img_size = np.array(img.size)

        # data augmentation for image
        center = np.array(img_size) / 2
        crop_size, crop_scale = img_size, 1
        random_flip_flag, random_crop_flag = False, False

        if self.data_augmentation:
            if np.random.random() < self.random_flip:
                random_flip_flag = True
                img = img.transpose(Image.FLIP_LEFT_RIGHT)

            if self.aug_crop:
                if np.random.random() < self.random_crop:
                    random_crop_flag = True
                    crop_scale = np.clip(
                        np.random.randn() * self.scale + 1, 1 - self.scale, 1 + self.scale)
                    crop_size = img_size * crop_scale
                    center[0] += img_size[0] * \
                                 np.clip(np.random.randn() * self.shift, -
                                 2 * self.shift, 2 * self.shift)
                    center[1] += img_size[1] * \
                                 np.clip(np.random.randn() * self.shift, -
                                 2 * self.shift, 2 * self.shift)

        # add affine transformation for 2d images.
        trans, trans_inv = get_affine_transform(
            center, crop_size, 0, self.resolution, inv=1)
        img = img.transform(tuple(self.resolution.tolist()),
                            method=Image.AFFINE,
                            data=tuple(trans_inv.reshape(-1).tolist()),
                            resample=Image.BILINEAR)

        img_weak_augmented = img.copy()

        if self.data_augmentation:
            if self.aug_pd:
                img = np.array(img).astype(np.float32)
                img = self.pd(img).astype(np.uint8)
                img = Image.fromarray(img)

        # image encoding
        img = np.array(img).astype(np.float32) / 255.0
        img = img[:, :, :3]
        img = (img - self.mean) / self.std
        img = img.transpose(2, 0, 1)  # C * H * W

        img_weak_augmented = np.array(img_weak_augmented).astype(np.float32) / 255.0
        img_weak_augmented = img_weak_augmented[:, :, :3]
        img_weak_augmented = (img_weak_augmented - self.mean) / self.std
        img_weak_augmented = img_weak_augmented.transpose(2, 0, 1)  # C * H * W

        info = {'img_id': index,
                'img_size': img_size,
                'resolution': self.resolution,
                'view': self.view_ext,
                'data_domain': 1}  # 1: target domain, 0: source domain

        if self.split == 'test':
            calib = self.get_calib(index)
            return img, calib.P2, img, info

        #  ============================   get labels   ==============================
        objects = self.get_label(index)
        calib = self.get_calib(index)
        extrinsics = self.get_extrinsics(index)

        yolo_dets = self._load_yolo_dets(index, int(img_size[0]), int(img_size[1]))
        per_obj_conf = self._match_yolo_conf(objects, yolo_dets)

        # data augmentation for labels
        if random_flip_flag:
            if self.aug_calib:
                calib.flip(img_size)
            for object in objects:
                [x1, _, x2, _] = object.box2d
                object.box2d[0], object.box2d[2] = img_size[0] - \
                                                   x2, img_size[0] - x1
                object.alpha = np.pi - object.alpha
                object.ry = np.pi - object.ry
                if self.aug_calib:
                    object.pos[0] *= -1
                if object.alpha > np.pi:
                    object.alpha -= 2 * np.pi  # check range
                if object.alpha < -np.pi:
                    object.alpha += 2 * np.pi
                if object.ry > np.pi:
                    object.ry -= 2 * np.pi
                if object.ry < -np.pi:
                    object.ry += 2 * np.pi

        # labels encoding
        calibs = np.zeros((self.max_objs, 3, 4), dtype=np.float32)
        indices = np.zeros((self.max_objs), dtype=np.int64)
        mask_2d = np.zeros((self.max_objs), dtype=bool)
        labels = np.zeros((self.max_objs), dtype=np.int8)
        depth = np.zeros((self.max_objs, 1), dtype=np.float32)
        heading_bin = np.zeros((self.max_objs, 1), dtype=np.int64)
        heading_res = np.zeros((self.max_objs, 1), dtype=np.float32)
        size_2d = np.zeros((self.max_objs, 2), dtype=np.float32)
        size_3d = np.zeros((self.max_objs, 3), dtype=np.float32)
        src_size_3d = np.zeros((self.max_objs, 3), dtype=np.float32)
        boxes = np.zeros((self.max_objs, 4), dtype=np.float32)
        boxes_3d = np.zeros((self.max_objs, 6), dtype=np.float32)
        ground = np.zeros((self.max_objs, 4), dtype=np.float32)
        conf_2d = np.zeros((self.max_objs,), dtype=np.float32)
        has_yolo_conf = np.zeros((self.max_objs,), dtype=np.float32)

        object_num = len(objects) if len(
            objects) < self.max_objs else self.max_objs

        for i in range(object_num):

            if objects[i].cls_type in self.skip_class_names:
                continue
            if objects[i].cls_type not in self.cls2id.keys():
                # print('filter out ', objects[i].cls_type)
                # raise ValueError('Unknown class type', objects[i].cls_type)
                continue

            obj_angles = np.array(objects[i].ry)

            # process 2d bbox & get 2d center
            bbox_2d = objects[i].box2d.copy()

            # add affine transformation for 2d boxes.
            bbox_2d[:2] = affine_transform(bbox_2d[:2], trans)
            bbox_2d[2:] = affine_transform(bbox_2d[2:], trans)

            # process 3d center
            center_2d = np.array(
                [(bbox_2d[0] + bbox_2d[2]) / 2, (bbox_2d[1] + bbox_2d[3]) / 2], dtype=np.float32)  # W * H
            corner_2d = bbox_2d.copy()

            # real 3D center in 3D space
            center_3d = objects[i].pos
            center_3d = center_3d.reshape(-1, 3)  # shape adjustment (N, 3)
            # project 3D center to image plane
            center_3d, _ = calib.rect_to_img(center_3d)
            center_3d = center_3d[0]  # shape adjustment
            if random_flip_flag and not self.aug_calib:  # random flip for center3d
                center_3d[0] = img_size[0] - center_3d[0]
            center_3d = affine_transform(center_3d.reshape(-1), trans)

            # class
            cls_id = self.cls2id[objects[i].cls_type]
            labels[i] = cls_id

            # encoding 2d/3d boxes
            w, h = bbox_2d[2] - bbox_2d[0], bbox_2d[3] - bbox_2d[1]
            size_2d[i] = 1. * w, 1. * h

            center_2d_norm = center_2d / self.resolution
            size_2d_norm = size_2d[i] / self.resolution

            corner_2d_norm = corner_2d
            corner_2d_norm[0: 2] = corner_2d[0: 2] / self.resolution
            corner_2d_norm[2: 4] = corner_2d[2: 4] / self.resolution
            center_3d_norm = center_3d / self.resolution

            l, r = center_3d_norm[0] - \
                   corner_2d_norm[0], corner_2d_norm[2] - center_3d_norm[0]
            t, b = center_3d_norm[1] - \
                   corner_2d_norm[1], corner_2d_norm[3] - center_3d_norm[1]

            boxes[i] = center_2d_norm[0], center_2d_norm[1], size_2d_norm[0], size_2d_norm[1]
            boxes_3d[i] = center_3d_norm[0], center_3d_norm[1], l, r, t, b

            # encoding depth
            if self.depth_scale == 'normal':
                depth[i] = objects[i].pos[-1] * crop_scale

            elif self.depth_scale == 'inverse':
                depth[i] = objects[i].pos[-1] / crop_scale

            elif self.depth_scale == 'none':
                depth[i] = objects[i].pos[-1]

            if obj_angles > np.pi:
                obj_angles -= 2 * np.pi
            elif obj_angles < -np.pi:
                obj_angles += 2 * np.pi
            # heading_bin[i, :], heading_res[i, :] = angles2class(obj_angles)
            heading_bin[i], heading_res[i] = angle2class(obj_angles)

            # encoding size_3d
            src_size_3d[i] = np.array(
                [objects[i].h, objects[i].w, objects[i].l], dtype=np.float32)
            mean_size = self.cls_mean_size[self.cls2id[objects[i].cls_type]]
            size_3d[i] = src_size_3d[i] - mean_size
            
            mask_2d[i] = 1
            calibs[i] = calib.P2 / 2
            ground[i] = extrinsics.extrinsics
            ground[i, 3] = ground[i, 3] / (-10)
            conf_2d[i] = float(per_obj_conf[i]) if i < len(per_obj_conf) else self.yolo_fallback_conf
            has_yolo_conf[i] = 1.0 if self.has_yolo_conf else 0.0

        # collect return data
        inputs = img
        targets = {
            'calibs': calibs,
            'indices': indices,
            'img_size': img_size,
            'labels': labels,
            'boxes': boxes,
            'boxes_3d': boxes_3d,
            'depth': depth,
            'size_2d': size_2d,
            'size_3d': size_3d,
            'src_size_3d': src_size_3d,
            'heading_bin': heading_bin,
            'heading_res': heading_res,
            'mask_2d': mask_2d,
            'ground': ground,
            'conf_2d': conf_2d,
            'has_yolo_conf': has_yolo_conf}

        info = {'img_id': index,
                'img_size': img_size,
                'resolution': self.resolution,
                'data_domain': 1,
                'view': self.view_ext,
                'img_weak_augmented': img_weak_augmented}  # 1: target domain, 0: source domain
        # print(f"Index {index}: objects = {len(objects)}, mask_2d sum = {mask_2d.sum()}, labels = {labels[mask_2d]}")
        return inputs, calib.P2, targets, info


if __name__ == '__main__':
    sys.path.append(os.getcwd())

    cfg = {
        'target_root_dir': '/home/ubuntu/Projects/lz_project/WARM-3D/data/s110_o_dataset',
        'random_flip': 0.5,
        'random_crop': 0.5,
        'scale': 0.05,
        'shift': 0.05,
        'use_dontcare': False,
        'class_merging': False,
        'writelist': ['Pedestrian', 'Car', 'Cyclist', 'Van', 'Truck', 'Bus', 'Motorcycle', 'Trailer'],
        'use_3d_center': True,
        'bbox2d_type': 'anno',
        'meanshape': False,
        'aug_pd': True,
        'aug_crop': True,
        'depth_scale': 'normal',
        'target_dataset_name': 'tum',
    }
    dataset = TargetDataset('train', cfg)
    dataloader = DataLoader(dataset=dataset, batch_size=4)

    for batch_idx, (inputs, calib, targets, info) in enumerate(dataloader):
        img = inputs[0].numpy().transpose(1, 2, 0)
        img = (img * dataset.std + dataset.mean) * 255
        img = Image.fromarray(img.astype(np.uint8))

        boxes = targets['boxes'][0]
        center_2d_norm = [boxes[:, 0], boxes[:, 1]]  # Assuming center_2d_norm is at index 1 and 2
        size_2d_norm = [boxes[:, 2], boxes[:, 3]]  # Assuming size_2d_norm is at index 3 and 4
        boxes_xyxy = box_ops.box_cxcywh_to_xyxy(boxes)
        boxes_xyxy = boxes_xyxy * torch.tensor([960, 600, 960, 600], dtype=boxes_xyxy.dtype)

        boxes_3d = targets['boxes_3d'][0]
        center_3d_norm = [boxes_3d[:, 0], boxes_3d[:, 1]]  # Assuming center_3d_norm is at index 1 and 2
        l, r, t, b = boxes_3d[:, 2], boxes_3d[:, 3], boxes_3d[:, 4], boxes_3d[:, 5]

        w, h = img.size
        center_2d = ((center_2d_norm[0] * w).to(torch.int), (center_2d_norm[1] * h).to(torch.int))
        center_3d = ((center_3d_norm[0] * w).to(torch.int), (center_3d_norm[1] * h).to(torch.int))

        heading_bin = targets['heading_bin'][0]
        heading_res = targets['heading_res'][0]
        angle = class2angles(heading_bin, heading_res, to_label_format=True)

        # Draw 2D and 3D centers on the image
        img_array = np.array(img)
        for i in range(center_2d[0].shape[0]):
            cv2.circle(img_array, (center_2d[0][i].item(), center_2d[1][i].item()), 5, (0, 255, 0), -1)

        for i in range(center_3d[0].shape[0]):
            cv2.circle(img_array, (center_3d[0][i].item(), center_3d[1][i].item()), 5, (0, 0, 255), -1)

        for j, box in enumerate(boxes_xyxy):
            cv2.rectangle(img_array, (int(box[0]), int(box[1])), (int(box[2]), int(box[3])), (255, 0, 0), 2)
            cv2.putText(img_array, f"{j}", (int(box[0]), int(box[1] - 10)), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255, 0, 0),
                        2)
        # Save the image with centers
        cv2.imwrite('output_image.png', cv2.cvtColor(img_array, cv2.COLOR_RGB2BGR))

        # Display the image
        img.show()

        break

    # print ground truth fisrt
    objects = dataset.get_label(0)
    for object in objects:
        print(object.to_kitti_plus_format())
