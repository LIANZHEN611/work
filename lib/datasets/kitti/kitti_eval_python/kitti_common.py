...
def get_label_anno(label_path):
    ...
    replace_items = ['Truck', 'Trailer']
    with open(label_path, 'r') as f:
        lines = f.readlines()
    #content = [line.strip().split(' ') for line in lines]
    content = [line.strip().split() for line in lines if line.strip()]
    for line_idx, fields in enumerate(content):
        if len(fields) < 15:
            raise ValueError(
                f"Invalid KITTI label format in {label_path} line {line_idx + 1}: "
                f"expected at least 15 fields (needs rotation_y), got {len(fields)}."
            )
