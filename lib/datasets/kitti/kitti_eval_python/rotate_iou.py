...
def rotate_iou_gpu_eval(boxes, query_boxes, criterion=-1, device_id=0):
    ...
    if boxes.ndim != 2 or boxes.shape[1] != 5:
        raise ValueError(
            f"rotate_iou_gpu_eval expects boxes shape [N, 5], but got {boxes.shape}. "
            "BEV boxes must be [x, z, l, w, ry]."
        )
    if query_boxes.ndim != 2 or query_boxes.shape[1] != 5:
        raise ValueError(
            f"rotate_iou_gpu_eval expects query_boxes shape [K, 5], but got {query_boxes.shape}. "
            "BEV boxes must be [x, z, l, w, ry]."
        )
    if not np.isfinite(boxes).all():
        raise ValueError("rotate_iou_gpu_eval received non-finite values in boxes.")
    if not np.isfinite(query_boxes).all():
        raise ValueError("rotate_iou_gpu_eval received non-finite values in query_boxes.")
