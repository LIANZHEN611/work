...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, outputs, targets, group_num=11):
        ...
        # 将 l,r,t,b 强制限制为非负（应对可能的极小负数）
        out_bbox_clamped = out_bbox.clamp(min=0)
        tgt_bbox_clamped = tgt_bbox.clamp(min=0)

        # 安全检查：去除任何包含 NaN/Inf 的框（同时打印警告）
        if torch.isnan(out_bbox_clamped).any() or torch.isinf(out_bbox_clamped).any():
            out_bbox_clamped = torch.nan_to_num(out_bbox_clamped, nan=0.0, posinf=0.0, neginf=0.0)
        if torch.isnan(tgt_bbox_clamped).any() or torch.isinf(tgt_bbox_clamped).any():
            tgt_bbox_clamped = torch.nan_to_num(tgt_bbox_clamped, nan=0.0, posinf=0.0, neginf=0.0)

        cost_giou = -generalized_box_iou(box_cxcylrtb_to_xyxy(out_bbox_clamped), box_cxcylrtb_to_xyxy(tgt_bbox_clamped))
        # Final cost matrix
        C = self.cost_bbox * cost_bbox + self.cost_3dcenter * cost_3dcenter + self.cost_class * cost_class + self.cost_giou * cost_giou
        # ===== 新增清理：彻底去除 NaN/Inf，并限制过大值 =====
        C = torch.nan_to_num(C, nan=0.0, posinf=1e6, neginf=-1e6)
        C = C.clamp(min=-1e6, max=1e6)  # 防止极端值
        # ================================================
...
class HungarianMatcherV2(HungarianMatcher):
    ...
    def forward(self, outputs, targets):
        ...
        out_bbox_clamped = out_bbox.clamp(min=0)
        tgt_bbox_clamped = tgt_bbox.clamp(min=0)
        out_bbox_clamped = torch.nan_to_num(out_bbox_clamped, nan=0.0, posinf=1.0, neginf=0.0)
        tgt_bbox_clamped = torch.nan_to_num(tgt_bbox_clamped, nan=0.0, posinf=1.0, neginf=0.0)
        cost_giou = -generalized_box_iou(box_cxcylrtb_to_xyxy(out_bbox_clamped), box_cxcywh_to_xyxy(tgt_bbox_clamped))
