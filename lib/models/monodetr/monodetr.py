...
from .additional_constraints import (calculate_3d_bbox_size,
                                     calculate_plane_constraint_loss)
...
class SetCriterion(nn.Module):
    ...
    @staticmethod
    def _conf_weights(targets, indices, device):
        if not any(isinstance(t, dict) and 'conf' in t for t in targets):
            return None
        w = torch.cat([t['conf'][i] for t, (_, i) in zip(targets, indices)], dim=0).to(device)
        return (w ** 2).clamp(min=1e-8)

    @staticmethod
    def _conf_reduce(loss_per_box, w, num_boxes):
        if w is None or w.numel() != loss_per_box.shape[0]:
            return loss_per_box.sum() / max(num_boxes, 1)
        return (loss_per_box * w).sum() / w.sum().clamp(min=1e-6)
    ...
    def loss_3dcenter(self, outputs, targets, indices, num_boxes):
        ...
        if num_boxes == 0:
            return {'loss_center': torch.tensor(0.0, device=outputs['pred_boxes'].device)}
        ...
        loss_3dcenter = F.l1_loss(src_3dcenter, target_3dcenter, reduction='none').sum(dim=-1)
        w = self._conf_weights(targets, indices, loss_3dcenter.device)
        losses = {'loss_center': self._conf_reduce(loss_3dcenter, w, num_boxes)}
        return losses
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        loss_bbox = F.l1_loss(src_2dboxes, target_2dboxes, reduction='none').sum(dim=-1)
        w = self._conf_weights(targets, indices, loss_bbox.device)
        losses = {'loss_bbox': self._conf_reduce(loss_bbox, w, num_boxes)}

        src_boxes = outputs['pred_boxes'][idx]
        target_boxes = torch.cat([t['boxes'][i] for t, (_, i) in zip(targets, indices)], dim=0)
        src_boxes = src_boxes.clamp(min=0)
        target_boxes = target_boxes.clamp(min=0)
        src_boxes = torch.nan_to_num(src_boxes, nan=0.0, posinf=1.0, neginf=0.0)
        ...
        losses['loss_giou'] = self._conf_reduce(loss_giou, w, num_boxes)
        return losses
    ...
    def loss_depths(self, outputs, targets, indices, num_boxes):
        if self.mask is not None:
            ...
            num_boxes = sum([len(i) for (i, j) in indices])
        if num_boxes == 0:
            return {'loss_depth': torch.tensor(0.0, device=outputs['pred_depth'].device)}

        idx = self._get_src_permutation_idx(indices)
        if idx[0].numel() == 0:   # ����
            return {'loss_depth': torch.tensor(0.0, device=outputs['pred_depth'].device)}
        src_depths = outputs['pred_depth'][idx]
        src_depth = src_depths[:, 0]
        target_depth = torch.cat([t['depth'][i] for t, (_, i) in zip(targets, indices)], dim=0).squeeze()

        loss_depth = torch.abs(src_depth - target_depth)
        w = self._conf_weights(targets, indices, loss_depth.device)
        losses = {'loss_depth': self._conf_reduce(loss_depth.view(-1), w, num_boxes)}
    ...
    def loss_dims(self, outputs, targets, indices, num_boxes):
        if self.mask is not None:
            ...
            num_boxes = sum([len(i) for (i, j) in indices])
        if num_boxes == 0:
            return {'loss_dim': torch.tensor(0.0, device=outputs['pred_3d_dim'].device)}
        ...
        dim_loss_per_box = dim_loss.sum(dim=-1)
        w = self._conf_weights(targets, indices, dim_loss_per_box.device)
        losses = {'loss_dim': self._conf_reduce(dim_loss_per_box, w, num_boxes)}
    ...
    def loss_angles(self, outputs, targets, indices, num_boxes):
        if self.mask is not None:
            ...
            num_boxes = sum([len(i) for (i, j) in indices])
        if num_boxes == 0:
            return {'loss_angle': torch.tensor(0.0, device=outputs['pred_angle'].device)}
        ...
        w = self._conf_weights(targets, indices, angle_loss.device)
        losses = {'loss_angle': self._conf_reduce(angle_loss, w, num_boxes)}
        return losses
...
def compute_weight_dict_target(cfg):
    ...
    weight_dict_target = {
        ...
        # ���� feature_extrinsic ��Ȩ��
        'loss_feature_extrinsic': cfg.get('target_feature_extrinsic_loss_coef', 1.0)
    }
