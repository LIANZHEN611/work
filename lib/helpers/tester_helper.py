...
class Tester(object):
    ...
    def inference(self):
        ...
        for batch_idx, (inputs, calibs, targets, info) in enumerate(self.dataloader):
            ...
            #����Ƿ��зǿյļ����������¼������
            non_empty = sum(1 for v in dets.values() if len(v) > 0)
            total_rows = sum(len(v) for v in dets.values())
            self.logger.info(
            f"[debug] batch={batch_idx} imgs={len(dets)} non_empty_imgs={non_empty} total_rows={total_rows}"
            )
    ...
    def save_results(self, results):
        ...
        skipped_non_finite = 0    # 情况 A 计数
        skipped_cls_oob    = 0    # 情况 B 计数
        ...
        for img_id in results.keys():
            ...
            for i in range(len(results[img_id])):
                if len(results[img_id][i]) < 16:
                    raise ValueError(
                        f"Invalid detection format for image {img_id}, index {i}: "
                        f"expected at least 16 total fields (class id + 15 KITTI attributes), "
                        f"got {len(results[img_id][i])}."
                    )
                row = results[img_id][i]
                #print(f"Row length: {len(row)}")
                arr = np.asarray(row, dtype=np.float64)
                if not np.all(np.isfinite(arr)):
                    if skipped_non_finite < 5:  # 只打印前 5 条
                        bad_cols = np.where(arr == 0)[0].tolist()
                        self.logger.warning(
                            f"[bad_row] img_id={img_id}, det_i={i}, "
                            f"bad_cols={bad_cols}, values={arr.tolist()}"
                        )
                    skipped_non_finite += 1
                    continue
                cls_idx = int(row[0])
                if cls_idx < 0 or cls_idx >= len(self.class_name):
                    skipped_cls_oob += 1
                    continue
                class_name = self.class_name[cls_idx]
                f.write('{} 0.0 0'.format(class_name))
                for j in range(1, len(row)):
                    f.write(' {:.2f}'.format(row[j]))
                f.write('\n')
            f.close()
        # 用这两行替换原来的 skipped_invalid_rows 日志
        if skipped_non_finite > 0 or skipped_cls_oob > 0:
            self.logger.warning(
                f"save_results: skipped_non_finite={skipped_non_finite}, "
                f"skipped_cls_oob={skipped_cls_oob}, "
                f"class_name={self.class_name} (len={len(self.class_name)})"
            )
        if self.train_cfg['save_all']:
    ...
    def set_tester_mode(self, mode):
        ...
        elif mode == 'teacher_burn_in':
            self.dataloader = self.dataloader_list[0]
            self.tester_mode = 'teacher_burn_in'
        elif mode == 'target_eval':
    ...
    def set_output_folder(self, camera_id):
        if camera_id is not None:
            ...
            self.result_dir = os.path.join(self.output_dir, 'outputs')
